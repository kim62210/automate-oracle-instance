...
## STEP 2. 키 파일 저장하기 (레포 폴더 안에 모아 두기 — 기본)

설정 파일과 비밀 키를 **이 스크립트 레포 폴더 안에 모아 두는 게 가장 편합니다.**
백업/이동도 폴더 하나만 옮기면 되고, `.gitignore` 가 자동으로 git push 에서 제외해 줍니다.

> 표준 위치(`~/.oci/config`)에 두고 싶다면 이 STEP 끝의 **부록 A** 참고.

먼저 이 스크립트를 받아 둡니다 (이미 받았으면 `cd` 만):
```bash
cd ~
...
> Git/Python 설치가 안 돼있어 위 명령이 실패하면 **STEP 5** 의 OS 별 설치 안내 참고.

### 2-1. 다운로드한 개인 키(.pem) 옮기기
...
```bash
mv ~/Downloads/<위에서_확인한_파일명>.pem ./oci_api_private_key.pem
...
먼저 현재 폴더의 절대경로를 확인 (다음 단계에서 사용):
```bash
pwd
# 출력 예: /Users/me/automate-oracle-instance
```
...
nano oci_config
```
- 1-2 단계에서 복사한 미리보기 내용을 **붙여넣기** (macOS: `command + v`)
- 마지막 줄 `key_file=...` 을 다음과 같이 바꿉니다 (위 `pwd` 결과 이용):
  ```
...
```bash
ls -la oci_config oci_api_private_key.pem
```
두 파일 모두 보이고 .pem 권한이 `-rw-------` 이면 OK.

---

### 부록 A. ~/.oci/config 에 두는 방법 (OCI CLI 와 호환)
...
```
이 경우 STEP 6 의 `oci.env` 에서 `OCI_CONFIG=~/.oci/config` 로 변경하면 됩니다.
...
## STEP 5. 도구 설치 안내 (필요 시)

> 💡 STEP 2 에서 이미 `git clone` 이 성공했으면 이 STEP 은 건너뛰어도 됩니다.

STEP 2 의 `git clone` 또는 STEP 9 의 `./setup_init.sh` 실행이 실패하면
git / Python 이 없어서 그렇습니다. OS 별 설치 방법:

**git 설치**:
- **macOS**: `git --version` 실행 시 Xcode Command Line Tools 설치 안내가 자동 출력 → 설치
- **Ubuntu/Debian/WSL**: `sudo apt update && sudo apt install -y git`
- **CentOS/RHEL**: `sudo dnf install -y git`

**Python 3 설치** (`python3 --version` 확인, 3.10 이상 권장):
...
실제로 본인이 채워야 할 항목은 보통 1-2 가지뿐입니다:

| 항목 | 채워야 할 때 |
|------|--------------|
| `OCI_SUBNET_ID` | **STEP 3-1 옵션 A/B 에서 메모해 둔 서브넷 OCID 붙여넣기 (강력 권장)**. 비워두면 자동 탐색 시도하지만 신규 계정은 실패함 |
| `DISCORD_WEBHOOK` | 알림 받고 싶을 때 (STEP 7 참고) |

기본값으로 동작하는 항목 (수정 불필요):
- `OCI_CONFIG=./oci_config` -- STEP 2 에서 레포 폴더 안에 만든 파일
...
> 🔄 STEP 2 부록 A 처럼 `~/.oci/config` 를 쓴 경우에만
> `OCI_CONFIG=~/.oci/config` 로 수정하세요.
...
> 💡 **Linux/WSL 첫 실행 시**: `sudo apt install python3-venv` 가 자동으로 실행되며,
> sudo 비밀번호 입력 프롬프트가 나타납니다. 본인 계정 비밀번호를 입력하세요.
> macOS 는 sudo 가 필요 없습니다.
...
| 변수 | 기본값 | 채워야 할 때 |
|------|--------|--------------|
| `OCI_CONFIG` | `./oci_config` | STEP 2 (레포 폴더 안)을 따랐다면 그대로. `~/.oci/config` 사용 시 변경 |
