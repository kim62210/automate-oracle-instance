...
`command not found` 가 나오면 OS 별로 설치:
- **macOS**: 위 명령 실행 시 Xcode Command Line Tools 설치 안내가 자동으로 뜸 → 설치 진행
- **Ubuntu/Debian**: `sudo apt update && sudo apt install -y git`
- **CentOS/RHEL**: `sudo dnf install -y git`
- **Windows (WSL Ubuntu)**: WSL 안에서 `sudo apt update && sudo apt install -y git`
...
Python 3 도 필요합니다. 보통 macOS / Ubuntu / WSL 에는 기본 설치돼 있지만,
없다면:
```bash
python3 --version   # 3.10 이상 권장. 'command not found' 면 아래 설치
